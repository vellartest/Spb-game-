...
const STORY_SCENES = {
    ...
    scene_startup_pitch: {
        ...
        lines: [
            ...
            { text: 'Ему нужен технический партнёр.', portrait: '💻' },
            { text: 'Ты соглашаешься!', portrait: '🤝' },
            { text: '+25 репутации! Теперь ты в деле.', portrait: '⭐' }
        ],
        reward: { rep: 25 },
    ...
    scene_finale: {
        ...
        lines: [
            ...
            { text: 'Осталось принять последнее решение...', portrait: '🤔', choices: [
                { text: '💰 Продать компанию за большие деньги', action: 'ending_rich' },
                { text: '🌱 Развивать проект и изменить мир', action: 'ending_wise' }
            ]}
        ]
    },
    ending_rich: {
        ...
        for (let i = 0; i < 25; i++) {
            ...
            for (let wy = GROUND_Y - bh + 10; wy < GROUND_Y - 10; wy += 18)
                for (let wx = bx + 10; wx < bx + 80; wx += 20)
                    if (((wx * 3 + wy + i) % 5) > 2) ctx.fillRect(wx, wy, 8, 10);
            ctx.fillStyle = '#1a0f2e';
        }
